/* ============================================================
   ...
   Cleaned and consolidated to remove corrupted/duplicated sections.
   ============================================================ */
...
function confidenceBadgeHTML(level) {
  const map = {
    ...
    2: { cls: 'c2', icon: 'fas fa-check-circle', label: 'Al-Minar Verified Masjid', color: '#059669', bg: '#D1FAE5' },
    3: { cls: 'c3', icon: 'fas fa-shield-alt', label: 'Al-Minar Verified Masjid', color: '#B8860B', bg: 'linear-gradient(135deg,#FEF3C7,#FDE68A)' },
    4: { cls: 'c4', icon: 'fas fa-crown', label: 'Al-Minar Verified Masjid', color: '#D4AF37', bg: 'linear-gradient(135deg,#FDE68A,#D4AF37)' },
  };
  const info = map[level] || map[0];
  // For C2, C3, C4, use custom inline style for color/bg if needed
  let style = '';
  if (level === 2) style = `background:${info.bg};color:${info.color}`;
  else if (level === 3) style = `background:${info.bg};color:${info.color};border:1px solid #F59E0B`;
  else if (level === 4) style = `background:${info.bg};color:${info.color};border:1px solid #D4AF37`;
  return `<span class="confidence-badge ${info.cls}" style="${style}"><i class="${info.icon}"></i> ${info.label}</span>`;
}
...
function masjidCardHTML(m) {
  ...
  const city = loc ? `${loc.city || ''}${loc.city && loc.country ? ', ' : ''}${loc.country || ''}` : 'Location unknown';
...
function initScrollAnimations() {
  ...
  } catch (e) { /* IntersectionObserver not supported — silently ignore */ }
}
...
async function loadFeatured() {
  ...
  try {
    ...
    container.innerHTML = '<div class="col-12 text-center py-5 text-danger">Error loading featured masjids.</div>';
  }
}
  ...
  try {
    ...
    if (prayerContainer && ptData.length > 0) {
      ...
      if (prayers.length > 0) {
        ...
        const prDateEl = document.getElementById('prayer-record-date');
        if (prDateEl) prDateEl.textContent = `Date: ${latest.date || 'N/A'} · Model: ${latest.modelType || 'N/A'}`;
      } else {
    ...
    if (mapEl && latEl && lngEl && typeof L !== 'undefined') {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        mapEl.innerHTML = '<div class="d-flex align-items-center justify-content-center h-100 text-muted"><div class="text-center"><i class="fas fa-map-marker-alt mb-2" style="font-size:2rem;opacity:.3;display:block;margin-bottom:.5rem"></i><p class="mb-0 small">No location data</p></div></div>';
      }
    }
...
async function initVerify() {
  ...
  try {
    ...
    const titleEl = card.querySelector('.verify-title');
    const bodyEl = card.querySelector('.verify-body');
    if (iconEl) { iconEl.className = 'verify-icon invalid'; iconEl.innerHTML = '<i class="fas fa-question"></i>'; }
    if (titleEl) { titleEl.textContent = 'Badge Not Found'; titleEl.className = 'verify-title text-danger fw-bold fs-4'; }
    if (bodyEl) bodyEl.innerHTML = '<p>No badge was found with this token.</p>';
  }
}
    ...
    try {
      ...
      try {
        ...
        if (adminRes.ok) {
          ...
          if (results.length > 0) userInfo.accountType = 'masjid_admin';
        }
      } catch {}
...
function initRegister() {
  ...
  document.querySelectorAll('.type-option').forEach(option => {
    option.addEventListener('click', () => {
      ...
      if (adminFields) { if (type === 'masjid_admin') adminFields.classList.remove('d-none'); else adminFields.classList.add('d-none'); }
    });
  });
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (tokenRes.ok) {
        ...
        setTimeout(() => { window.location.href = accountType === 'masjid_admin' ? '/report/' : '/'; }, 1200);
      } else {
        alert.className = 'auth-alert alert-success';
...
function initReportMap() {
  ...
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap', maxZoom: 18 }).addTo(reportMap);

  let marker = null;
  reportMap.on('click', (e) => {
    const { lat, lng } = e.latlng;
    const latIn = document.getElementById('report-lat');
    const lngIn = document.getElementById('report-lng');
...
function initPDFUpload() {
  ...
  dropZone.addEventListener('drop', (e) => {
    ...
    if (e.dataTransfer.files.length > 0) { fileInput.files = e.dataTransfer.files; showPDFName(e.dataTransfer.files[0].name); }
  });

  fileInput.addEventListener('change', () => { if (fileInput.files.length > 0) showPDFName(fileInput.files[0].name); });
}

function showPDFName(name) {
  const el = document.getElementById('pdf-file-name');
  const text = document.getElementById('pdf-name-text');
  if (el && text) { text.textContent = name; el.classList.remove('d-none'); }
}
...
async function submitMasjidReport(isAdmin) {
  ...
  try {
    const masjidRes = await authFetch(`${API}/masjids/`, {
      ...
      body: JSON.stringify({ name, description: desc || null, isActive: isAdmin }),
    });
    ...
    // Create location record (best-effort)
    await authFetch(`${API}/location-records/`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ masjid: masjid.masjidID, latitude: lat, longitude: lng, city, country, region: region || null }),
    }).catch(() => {});

    const user = Auth.getUser();
    const signalBody = { masjid: masjid.masjidID, signalType: isAdmin ? 'ADMIN_VERIFY' : 'ACTIVE', sourceType: isAdmin ? 'ADMIN' : 'USER', description: isAdmin ? 'Masjid admin direct submission' : 'Community report — initial signal' };
    if (user?.username) signalBody.user = user.username;

    await authFetch(`${API}/signals/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(signalBody) });

    if (isAdmin && user?.username) {
      await authFetch(`${API}/masjid-admins/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user: user.username, masjid: masjid.masjidID, verifiedIdentity: false }) }).catch(() => {});
    }

    if (alert) {
      alert.className = 'auth-alert alert-success';
      alert.innerHTML = `<i class="fas fa-check-circle me-2"></i>Masjid "<strong>${escHTML(name)}</strong>" ${isAdmin ? 'added successfully! It is now live on the site.' : 'reported! It will appear on the site once 3 community members verify it.'}`;
      alert.classList.remove('d-none');
    }
    document.getElementById('report-form')?.reset();

  } catch (err) {
    if (alert) { alert.className = 'auth-alert alert-danger'; alert.innerHTML = `<i class="fas fa-exclamation-circle me-2"></i>${escHTML(err.message)}`; alert.classList.remove('d-none'); }
  } finally {
    if (btn) { btn.querySelector('.btn-text')?.classList.remove('d-none'); btn.querySelector('.btn-loading')?.classList.add('d-none'); btn.disabled = false; }
  }
}
...
async function loadPendingMasjids() {
  ...
  try {
    ...
    grid.innerHTML = masjids.slice(0, 6).map(m => {
      const loc = m.location_record; const city = loc ? `${loc.city || ''}${loc.city && loc.country ? ', ' : ''}${loc.country || ''}` : 'Unknown';
      return `
        <div class="col-md-6 col-lg-4 mb-3">
          <div class="pending-card">
            ...
            <div class="card-location mb-2"><i class="fas fa-map-marker-alt"></i><span>${escHTML(city)}</span></div>
            <p class="text-muted mb-2" style="font-size:.82rem">${escHTML(m.description || 'No description.')}</p>
            <button class="btn btn-al-primary btn-sm w-100 verify-pending-btn" data-masjid-id="${m.masjidID}"><i class="fas fa-check me-1"></i>I Can Verify This Masjid</button>
          </div>
        </div>`;
    }).join('');
    ...
    grid.querySelectorAll('.verify-pending-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const masjidId = btn.dataset.masjidId; const user = Auth.getUser();
        btn.disabled = true; btn.innerHTML = '<div class="al-spinner" style="width:16px;height:16px;margin:0 auto;border-width:2px"></div>';
        try {
          await authFetch(`${API}/signals/`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ masjid: masjidId, signalType: 'ACTIVE', sourceType: 'USER', description: 'Community verification signal', user: user?.username || null }) });
          btn.className = 'btn btn-success btn-sm w-100'; btn.innerHTML = '<i class="fas fa-check me-1"></i>Verified — Thank You!';
        } catch (err) {
          btn.className = 'btn btn-outline-danger btn-sm w-100'; btn.innerHTML = '<i class="fas fa-times me-1"></i>Failed — Try Again'; btn.disabled = false;
        }
      });
    });

    initScrollAnimations();
  } catch (err) { console.error('Failed to load pending masjids:', err); }
}
